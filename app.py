...
MODEL = st.secrets.get("MODEL", "claude-haiku-4-5")
...
def extract_prompt(doc: str) -> str:
    return (
        ...
        "7) 세로줄(|)로 구분된 줄은 표의 행이다. 표 안에서 기간·대상·장소·규모·예산 등 핵심 팩트를 찾아 짝지어라.\n\n"
        '기획안:\n"""\n' + doc + '\n"""'
    )
...
def body_prompt(title: str, facts: dict) -> str:
    ...
    if no_quote:
        ...
        rule3 = "3) 인용문은 팩트의 '인용방향'을 반영해 초안으로 자연스럽게 작성한다."
        out_fmt = "마지막 문단이 인용."
    return (
...
_passcode = str(st.secrets.get("PASSCODE", "") or "")
